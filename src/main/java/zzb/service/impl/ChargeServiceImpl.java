...
public class ChargeServiceImpl implements ChargeService {
    ...
    public Result<Empty> addOneCharge(String chargeAccount, String chargePassword, String chargeName,String email) {
        try {
            // 检查账号是否存在
            Charge existing = chargeDao.getOneCharge(chargeAccount);
            if (existing != null) {
                return new Result<>("账号已存在");
            }

            chargeDao.addOneAdministrator(chargeAccount, chargePassword, chargeName, email);
    ...
    @Override
    @Transactional
    public Result<Empty> modifyOneCharge(String chargeAccount, String chargePassword) {
        try {
            chargeDao.updateOneCharge(chargeAccount, chargePassword);
